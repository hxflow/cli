...
| `hx migrate [--dry-run]` | 迁移老版本安装产物并重跑 setup |
| `hx version` | 查看版本号 |
...
执行 `hx setup` 时会生成各 agent 的 skill 入口；`hx setup` 首次会列出 agent 清单供用户选择，后续默认复用 `~/.hx/settings.yaml` 中记录的 agents。业务侧自定义 skill 仍由用户自行管理。
