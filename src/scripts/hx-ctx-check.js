...
const profileName =
  typeof options.profile === "string"
    ? options.profile
    : ctx.defaultProfile || getDefaultProfile(ROOT);
...
const refs = [
  ...agentsContent.matchAll(/→\s+([\w/\-.]+\.(?:md|json|js|ts))/g),
].map((match) => match[1]);
let validRefs = 0;
...
const planPathPattern = /docs\/plans\/([\w-]+)\.md/g;
const activeFeatures = [
  ...new Set(
    [...agentsContent.matchAll(planPathPattern)].map((match) => match[1]),
  ),
];
...
const progressEntries = findProgressFiles(ROOT, { plansDir: ctx.plansDir }).map(
  (filePath) => ({ filePath, data: readJsonFile(filePath) }),
);
...
for (const { filePath, data } of filteredProgress) {
  ...
  const requirementDoc =
    typeof data.requirementDoc === "string" ? data.requirementDoc : null;
  if (!requirementDoc || !existsSync(resolve(ROOT, requirementDoc))) {
    errors.push(
      `进度文件引用的需求文档不存在: ${relativePath(filePath)} -> ${requirementDoc || "未填写"}`,
    );
    continue;
  }
...
if (profileName) {
  try {
    const profile = loadProfile(FRAMEWORK_ROOT, profileName, { searchRoots: buildProfileSearchRoots(ctx.projectRoot) });
    const requiredFiles = [
    ...
    if (profile.files.platformPath && !existsSync(profile.files.platformPath)) {
      errors.push(
        `平台 profile 缺失: ${relativePath(profile.files.platformPath)}`,
      );
    }
...
function checkRequiredAbsDoc(
  ...
  const displayPath =
    label ||
    absolutePath.replace(ROOT + "/", "").replace(FRAMEWORK_ROOT + "/", "");
