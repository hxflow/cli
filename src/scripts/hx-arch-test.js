...
const layerEntries = profile.architecture.layers
  ...
  .filter(
    (layer) =>
      typeof layer.path === "string" && layer.path && !layer.path.includes("{"),
  );
...
const rules = layerEntries.map((layer) => {
  ...
  const forbiddenLayers = layerEntries.filter(
    (candidate) => !allowed.has(candidate.id),
  );
  return {
    layer,
...
for (const rule of rules) {
  ...
  for (const filePath of files) {
    ...
    for (const importPath of imports) {
      ...
      const banned = rule.forbids.find((candidate) =>
        pathMatchesLayer(target, candidate.path),
      );
      if (!banned) {
        continue;
      }
...
console.log(
  `✓ 架构合规检查通过（${profile.profile}，检查 ${checkedFiles} 个文件）`,
);
...
function resolveImport(filePath, importPath) {
  ...
  if (importPath.startsWith(".")) {
    return normalisePath(
      relative(ROOT, resolve(ROOT, dirname(filePath), importPath)),
    );
  }
