...
export function updateAgentsActiveFeature(root, featureName, profile, opts = {}) {
  ...
  const planMdRelPath = relative(projectRoot, resolve(plansDir, `${featureName}.md`))
  const featureLine = `→ ${planMdRelPath}（${profile.label}${profile.platformLabel ? ` · ${profile.platformLabel}` : ''}，状态：pending）`
  if (current.includes(planMdRelPath)) {
    return false
  }
