  ...
  for (const ref of refs) {
    ...
    if (srcMtime > docMtime && srcMtime > 0 && docMtime > 0) {
      stale.push({
        ...
        src: ref,
        docDate: docMtime
          ? new Date(docMtime * 1000).toLocaleDateString("zh-CN")
          : "未知",
        srcDate: srcMtime
          ? new Date(srcMtime * 1000).toLocaleDateString("zh-CN")
          : "未知",
      });
    }
  }
...
console.log(
  "  2. 或运行 hx:run -- <feature> <task-id> --profile <team> 让 Agent 读取代码后更新文档",
);
