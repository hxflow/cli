...
import { resolveContext, FRAMEWORK_ROOT, buildProfileSearchRoots } from "./lib/resolve-context.js";
import { loadHooks, injectHooks } from "./lib/hooks-utils.js";
...
if (!parsed.taskId) {
  console.error(
    "用法: npm run hx:run -- <feature-name> <TASK-ID> [--profile <team[:platform]>]",
  );
  console.error(
    "示例: npm run hx:run -- user-login TASK-BE-03 --profile backend",
  );
  process.exit(1);
}

const progressEntry = findProgressByTask(
  ROOT,
  parsed.taskId,
  parsed.featureName,
...
const profileName =
  typeof options.profile === "string"
    ? options.profile
    : inferProfileFromProgress(progressEntry.data) ||
      guessProfileFromTaskId(parsed.taskId) ||
      ctx.defaultProfile ||
      getDefaultProfile(ROOT);

let profile;
try {
  profile = loadProfile(FRAMEWORK_ROOT, profileName, { searchRoots: buildProfileSearchRoots(ctx.projectRoot) });
} catch (error) {
...
if (expectedProfile && expectedProfile !== profile.profile) {
  console.error(
    `✗ 计划文件中的 profile 为 ${expectedProfile}，与当前参数 ${profile.profile} 不一致`,
  );
  process.exit(1);
}
...
const requirementPath = progressEntry.data.requirementDoc
  ? resolve(ROOT, progressEntry.data.requirementDoc)
  : resolve(ctx.requirementDir, `${featureName}.md`);
const requirementRelPath = relativePath(requirementPath);
const planPath = resolve(ctx.plansDir, `${featureName}.md`);
const planRelPath = relativePath(planPath);
...
const platformRelPath = profile.files.platformPath
  ? relativePath(profile.files.platformPath)
  : null;

const contextFiles = [
  relativePath(ctx.agentsPath),
  relativePath(ctx.goldenPrinciplesPath),
  goldenRulesRelPath,
  profileRelPath,
  platformRelPath,
  planRelPath,
...
const layerSummary = profile.architecture.layers
  .map((layer) => {
    const allowed =
      Array.isArray(layer.can_import) && layer.can_import.length > 0
...
const gateCommands =
  Object.entries(profile.gateCommands)
    .map(([step, command]) => `- ${step}: ${command}`)
    .join("\n") || "- 未定义";

const hooks = loadHooks(ctx, 'run')

const divider = "═".repeat(60);
console.log(`\n${divider}`);
console.log(
  `  Agent Prompt — ${task.id} (${profile.label}${profile.platformLabel ? ` · ${profile.platformLabel}` : ""})`,
);
...
console.log(injectHooks(prompt, hooks));
console.log();
console.log(divider);
console.log();
