...
const profileName =
  typeof options.profile === "string"
    ? options.profile
    : ctx.defaultProfile || getDefaultProfile(ctx.projectRoot);
let profile;
try {
...
const hooks = loadHooks(ctx, 'fix')

const divider = "═".repeat(60);
const goldenRulesPath = profile.files.goldenRulesPath.replace(
  `${FRAMEWORK_ROOT}/`,
  "",
);
...
console.log(`\n${divider}`);
console.log(
  `  Bug 修复 Prompt 生成器 (${profile.label}${profile.platformLabel ? ` · ${profile.platformLabel}` : ""})`,
);
console.log(divider);
console.log();
...
console.log(
  `  --profile=${profile.profile} 指定团队约束（默认 ${profileName}）`,
);

function relativePath(filePath) {
  return filePath
    .replace(`${ctx.projectRoot}/`, "")
    .replace(`${FRAMEWORK_ROOT}/`, "");
}
