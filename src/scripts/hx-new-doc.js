...
// 用法: hx doc <feature-name> [--profile backend|frontend|mobile:ios]
// 从 profile 模板创建需求文档
...
if (!featureName) {
  console.error("用法: hx doc <feature-name> [--profile <team[:platform]>]");
  console.error(
    `示例: hx doc user-login --profile ${profileName || "backend"}`,
  );
  process.exit(1);
}
...
console.log(
  `✓ 需求文档已创建: ${outputPath.replace(ctx.projectRoot + "/", "")}`,
);
console.log(
