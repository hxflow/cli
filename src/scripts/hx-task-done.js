...
import { inferProfileFromProgress, isTaskId, parseArgs } from './lib/profile-utils.js'
...
if (!taskId) {
  console.error('用法: hx done <TASK-ID>')
  console.error('示例: hx done TASK-BE-03')
  process.exit(1)
}
...
for (const filePath of progressFiles) {
  try {
    ...
    if (doneCount === data.tasks.length) {
      ...
      console.log('  记得运行 hx entropy 检查是否有可沉淀的规则')
      break
    }
    ...
    if (nextTask) {
      ...
      console.log(
        `  hx run ${data.feature} ${nextTask.id}${profile ? ` --profile ${profile}` : ''}`
      )
    }
    break
  } catch (error) {
