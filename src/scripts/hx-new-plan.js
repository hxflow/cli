...
// 用法: hx plan <feature-name> [--profile backend|frontend|mobile:ios]
...
if (!featureName) {
  console.error('用法: hx plan <feature-name> [--profile <team[:platform]>]')
...
if (!existsSync(requirementPath)) {
  ...
  console.error('  请先运行 hx doc 创建文档')
  process.exit(1)
}
...
const agentsUpdated = updateAgentsActiveFeature(FRAMEWORK_ROOT, featureName, profile, { agentsPath: ctx.agentsPath, plansDir: ctx.plansDir, projectRoot: ctx.projectRoot })
