...
import { parseArgs, parseProfileSpecifier, profileUsage, readHxConfig } from './lib/profile-utils.js'
...
try {
  ...
  console.error('  示例: hx init -t /path/to/project -p backend')
  process.exit(1)
}

const existingConfig = readHxConfig(targetRoot)
...
function ResultView({ result }) {
  return React.createElement(
    ...
    React.createElement(Box, { flexDirection: 'column', marginTop: 1 },
      ...
      React.createElement(Text, null, `- hx doc <feature-name> --profile ${result.defaultProfile}`),
...
function performInstall({ targetRoot, defaultProfile, shouldInstallDeps, packageManager }) {
  ...
  writeHxConfigFile(targetRoot, defaultProfile, summary)
...
function copyFrameworkFiles(targetRoot, defaultProfile, summary) {
  ...
  const managedEntries = [
    { source: resolve(SOURCE_ROOT, 'agents', 'AGENTS.md'), target: resolve(targetRoot, 'AGENTS.md') },
    { source: resolve(SOURCE_ROOT, 'docs'), target: resolve(targetRoot, 'docs') },
    { source: resolve(SOURCE_ROOT, 'profiles', 'base'), target: resolve(targetRoot, '.hx', 'profiles', 'base') },
    { source: resolve(SOURCE_ROOT, 'profiles', team), target: resolve(targetRoot, '.hx', 'profiles', team) },
    ...
    { source: resolve(SOURCE_ROOT, 'agents', 'commands'), target: resolve(targetRoot, '.claude', 'commands') },
    { source: resolve(SOURCE_ROOT, 'agents', 'skills'), target: resolve(targetRoot, '.claude', 'skills') },
...
function writeHxConfigFile(targetRoot, defaultProfile, summary) {
  const hxDir = resolve(targetRoot, '.hx')
  const configPath = resolve(hxDir, 'config.json')
  if (!existsSync(hxDir)) mkdirSync(hxDir, { recursive: true })
  const nextConfig = {
    ...readHxConfig(targetRoot),
    defaultProfile
  }
  writeJson(configPath, nextConfig)
  summary.updated.push('.hx/config.json')
}
...
function printPlainSummary(result) {
  ...
  console.log(`- hx doc <feature-name> --profile ${result.defaultProfile}`)
