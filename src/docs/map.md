...
- 后端任务读取当前解析结果中的 `backend/profile.yaml`
- 前端任务读取当前解析结果中的 `frontend/profile.yaml`
- 移动端任务读取当前解析结果中的 `mobile/profile.yaml`，再叠加 `mobile/platforms/*.yaml`
...
- 新增团队规则时，先更新对应的 profile 配置文件
- 新增或修改需求时，使用 `hx doc <feature>` 从模板生成，或直接编辑 `docs/requirement/*.md`
- 新增或修改任务拆分时，使用 `hx plan <feature>` 自动生成 `docs/plans/*.md` 与 `*-progress.json`
- 自定义 profile 放在项目 `.hx/profiles/<name>/` 目录，通过 `extends: base` 或 `extends: backend` 继承通用规则
