...
最小迁移成本：只需新建 `.hx/config.yaml` + `.hx/profiles/backend-go/`，不改现有目录结构和 Skill。
...
✓ /hx-doc user-login 完成
  docs/requirement/user-login.md
...
下一步: /hx-run user-login TASK-BE-05 --profile backend
```
...
8 个步骤：doc → plan → ctx → run → review → gate → entropy → mr
...
当前 `/hx-run-all` 已实现类似功能，但未集成到 pipeline YAML 语法中。
