...
2. 生成或续接 `docs/requirement/{feature}.md`。执行 `bun scripts/tools/doc.ts validate <feature>`；若无 bun 则执行 `npx tsx scripts/tools/doc.ts validate <feature>`。根据校验结果修正头部和正文。
